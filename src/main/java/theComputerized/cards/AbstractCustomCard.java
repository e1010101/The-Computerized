...
import theComputerized.util.CardArtRoller;

import java.util.ArrayList;
import java.util.List;

import static theComputerized.Main.makeImagePath;
import static theComputerized.Main.modID;
...
public abstract class AbstractCustomCard extends CustomCard {
    ...
    public AbstractCustomCard(final String cardID, final int cost,
        ...
        super(cardID, "",
              getCardTextureString(cardID.replace(modID + ":", ""), type), cost,
        ...
        if (textureImg.contains("ui/missing.png")) {
            if (CardLibrary.getAllCards() != null &&
                !CardLibrary.getAllCards().isEmpty()) {
    ...
    public static String getCardTextureString(final String cardName,
                                              final AbstractCard.CardType cardType) {
    ...
    protected void dmg(AbstractMonster m, AbstractGameAction.AttackEffect fx) {
        atb(new DamageAction(m, new DamageInfo(AbstractDungeon.player, damage,
                                               damageTypeForTurn), fx));
    }

    protected void dmgTop(AbstractMonster m,
                          AbstractGameAction.AttackEffect fx) {
        att(new DamageAction(m, new DamageInfo(AbstractDungeon.player, damage,
                                               damageTypeForTurn), fx));
    }

    protected void allDmg(AbstractGameAction.AttackEffect fx) {
        atb(new DamageAllEnemiesAction(AbstractDungeon.player, multiDamage,
                                       damageTypeForTurn, fx));
    }

    protected void altDmg(AbstractMonster m,
                          AbstractGameAction.AttackEffect fx) {
        atb(new DamageAction(m, new DamageInfo(AbstractDungeon.player,
                                               secondDamage, damageTypeForTurn),
                             fx));
    }

    protected void blck() {
        atb(new GainBlockAction(AbstractDungeon.player, AbstractDungeon.player,
                                block));
    }
    ...
    public void setCustomTooltips(List<TooltipInfo> tooltips) {
        customTooltips.addAll(tooltips);
    }
}
